    ...
    for mn in results:
        ...
        for i in range(0,2):
            ...
            p.append(plt.bar(rt,ss[i],w,color=colors[i],bottom=b,
                             edgecolor="none"))
        plt.ylabel('HP remaining (Stingwing shown as negative)')
...
def main():
    ...
    results=dict()
    mname="Raider vs. Stingwing"
    results[mname]=dict()
    results[mname]["by_victor"]=dict()

    enemylist=[ "Raider Leader", "Stingwing" ]
    ...
    d1=catalog["creature"]["stingwing"].copy()
    d1.recalc_skills()
    d1.name=enemylist[1]
    df=FalloutSimulator.Faction.Faction(name="Stingwing")
    d1.factions=[df]
